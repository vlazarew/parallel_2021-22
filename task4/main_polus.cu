...
sendRecvTotal(const vector<double> &u, const vector <ProcessParallelepiped> &parallelepipeds,
              int processId, int countOfProcesses, ProcessParallelepiped target, int layerSize, int N) {
    if (processId != MAIN_PROCESS_ID) {
        ...
        MPI_Isend(u.data(), target.size, MPI_DOUBLE, MAIN_PROCESS_ID, 0, MPI_COMM_WORLD, &request);
    ...
    vector<double> uAll(layerSize);
    ProcessParallelepiped parallelepipedAll = createParallelepiped(0, N, 0, N, 0, N);

    for (int index = 0; index < countOfProcesses; index++) {
...
void fillAnalyticalValues(vector<double> &u, double t, double hx, double hy, double hz, Parallelepiped L, ProcessParallelepiped target) {
    for (int x = target.xMin; x <= target.xMax; x++) {
        for (int y = target.yMin; y <= target.yMax; y++) {
            for (int z = target.zMin; z <= target.zMax; z++) {
                u[getIndex(x, y, z, target)] =
                        getAnalyticValue(x * hx, y * hy, z * hz, t, L);
            }
        }
    }
}
...
void fillDifferenceValues(vector<double> &u, double t, double hx, double hy, double hz, Parallelepiped L, ProcessParallelepiped target) {
    for (int x = target.xMin; x <= target.xMax; x++) {
        for (int y = target.yMin; y <= target.yMax; y++) {
            for (int z = target.zMin; z <= target.zMax; z++) {
                u[getIndex(x, y, z, target)] =
                        fabs(u[getIndex(x, y, z, target)]) -
                        getAnalyticValue(x * hx, y * hy, z * hz, t, L);
            }
        }
    }
...
__global__ void evaluateErrorKernel(double *u, double hx, double hy, double hz, double t, Parallelepiped L,
    ...
    for (int index = idx; index < target.size; index += GPU_BLOCK_SIZE) {
...
void saveValues(const vector<double> &u, double t, const vector <ProcessParallelepiped> &parallelepipeds,
                int processId, int countOfProcesses, ProcessParallelepiped target, int layerSize, Parallelepiped L, int N,
                const char *filename) {
    vector<double> uAll = sendRecvTotal(u, parallelepipeds, processId, countOfProcesses, target, layerSize, N);
...
double makeSolution(Parallelepiped L, double T, int N, int K, int processId, int countOfProcesses, int steps) {
    double hx = L.x / N;
    double hy = L.y / N;
    double hz = L.z / N;
    double tau = T / K;
    ...
    vector <ProcessParallelepiped> parallelepipeds = splitParallelepiped(N, countOfProcesses);

    // Запоминаем рабочий параллелепипед
    ProcessParallelepiped target = parallelepipeds[processId];

    // Заполняем соседей
    fillNeighbours(parallelepipeds, processIds, send, recv, processId, countOfProcesses, target);

    vector <vector<double>> u(3, vector<double>(target.size));

    // Заполняем начальнные условия (step 4)
    fillVectorByInitialValues(u[0], u[1], hx, hy, hz, tau, L, N, processIds, send, recv, target);
...
//    }
    ...
    double error0 = evaluateError(u[0], 0, hx, hy, hz, L, target);
    double error1 = evaluateError(u[1], tau, hx, hy, hz, L, target);
    ...
    for (int step = 2; step <= steps; step++) {
        ...
        fillNextLayer(u[(step + 1) % 3], u[(step + 2) % 3], u[step % 3], step * tau, hx, hy, hz, tau, L, N,
                      processIds, send, recv, target);

        // Вычисляем максимальную ошибку (step 8)
        double error = evaluateError(u[steps % 3], step * tau, hx, hy, hz, L, target);
        // if (processId == MAIN_PROCESS_ID) {
    ...
    //saveValues(u[steps % 3], t, parallelepipeds, processId, countOfProcesses, target, layerSize, L, N,  "numerical.json");

    //fillDifferenceValues(u[steps % 3], t,  hx, hy, hz, L, target);
    //saveValues(u[steps % 3], t, parallelepipeds, processId, countOfProcesses, target, layerSize, L, N, "difference.json");

    //fillAnalyticalValues(u[0], t, hx, hy, hz, L, target);
    //saveValues(u[0], t, parallelepipeds, processId, countOfProcesses, target, layerSize, L, N,"analytical.json");
    //

    return evaluateError(u[steps % 3], t, hx, hy, hz, L, target);
}
...
int main(int argc, char *argv[]) {
    ...
    for (size_t i = 0; i < loops; i++) {
        error += makeSolution(variables.L, variables.T, variables.N, variables.K, processId, countOfProcesses, variables.steps);
    }
    ...
    if (processId == MAIN_PROCESS_ID) {
        ...
        cout << "Final error: " << error / loops << endl;
        cout << "Maximum time (s): " << maxTime / loops << endl;
        cout << "MPI processes: " << variables.countOfProcesses << endl;
    }
