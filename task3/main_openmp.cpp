...
void initVariables(int argc, char *argv[], SolverVariables &variables, int processId, int countOfProcesses) {
    ...
    variables.layerSize = (variables.N + 1) * (variables.N + 1) * (variables.N + 1);
...
double getAnalyticValue(double x, double y, double z, double t, const Parallelepiped &L) {
    double at = M_PI * sqrt(1 / (L.x * L.x) + 1 / (L.y * L.y) + 4 / (L.z * L.z));
...
vector<double>
sendRecvTotal(vector<double> u, const vector<ProcessParallelepiped> &parallelepipeds,
              const SolverVariables &variables) {
    if (variables.processId != MAIN_PROCESS_ID) {
        MPI_Request request;
        MPI_Status status;
...
int main(int argc, char *argv[]) {
    ...
    if (processId == MAIN_PROCESS_ID) {
        ...
        cout << "MPI threads: " << variables.countOfProcesses<< endl;
	    cout << "N: " << variables.N<< endl;
	    cout << "OMP threads: " << variables.ompThreadsCount << endl;
        cout << "Final error: " << error / loops << endl;
        cout << "Minimal time (s): " << minTime / loops << endl;
        cout << "Maximum time (s): " << maxTime / loops << endl;
        cout << "Average time (s): " << avgTime / loops << endl << endl;
